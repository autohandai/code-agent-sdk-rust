...
impl CurrentCliFixture {
    async fn start(result: &str, notification: &str) -> Self {
        ...
        // A checked-in executable avoids inherited writer descriptors during
        // concurrent test process creation on Linux (ETXTBSY).
        let cli = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/current_cli.sh");
